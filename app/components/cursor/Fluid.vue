...
onMounted(() => {
  ...
  let dye: ReturnType<typeof createDoubleFBO>,
    ...
    divergence: ReturnType<typeof createFBO>,
    curl: ReturnType<typeof createFBO>,
    pressure: ReturnType<typeof createDoubleFBO>;
  ...
  function drawDisplay(target: any) {
    ...
    if (config.SHADING) {
      gl.uniform2f(displayMaterial.uniforms, 1.0 / width, 1.0 / height);
    }
    gl.uniform1i(displayMaterial.uniforms, dye.read.attach(0));
    blit(target);
  }
  ...
  function clickSplat(pointer: any) {
    ...
    color.g && (color.g *= 10.0);
    color.b && (color.b *= 10.0);
